...
class ControlCheck(Control):
    ...
    def update_display(self):
        # Bugfix relative to upstream lightparam/stytra's vendored copy:
        # QCheckBox has no setValue - this crashed any refresh_widgets() call
        # on a checkbox-backed Param (previously never exercised in sashimi,
        # since its only other boolean Param uses gui=False).
        self.control.setChecked(self.param.value)
